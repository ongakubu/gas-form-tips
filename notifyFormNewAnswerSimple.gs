...
/*シンプル版：単純にフォームの回答内容が文字列で通知される。回答者への回答コピー送信、回答内容の設問別取得・判別・条件分岐等、一切無し。*/
...
function notifyFormNewAnswerSimple(e) {
  ...
  const mailTo        = 'recipient1@example.com'; /*主たる受取人（Google App Scriptの制限に抵触しないようToに設定するメールアドレスの数は最小限に抑えたほうがよい）*/
